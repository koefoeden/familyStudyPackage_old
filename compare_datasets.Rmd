...
participants_per_project <- map(projects, ~pheno_data_wide %>% 
                                pull(sourceId) %>% 
                                unique())


participants_clinical <- get_clinical_data() %>% pull(ID) %>% unique() %>% list() %>% set_names("clinical")
# the RNA seq data
...
# result
participants_per_project_w_rna_seq <- c(participants_per_project,participants_rna_seq, participants_clinical)
